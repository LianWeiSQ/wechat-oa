...
async function migrateArticles() {
  for (const row of all("select * from articles")) {
    await upsert("articles", {
      ...
      source_project: row.source_project || row.source_name || row.source_account || "",
...
async function migrateDrafts() {
  for (const row of all("select * from drafts")) {
    await upsert("drafts", {
      ...
      source_article_ids: parseJson(row.source_article_ids_json, []),
      content_channel: row.content_channel || "wechat",
      publish_status: row.publish_status || (row.wechat_draft_status === "sent" ? "published" : "draft"),
